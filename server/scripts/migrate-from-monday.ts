...
${report.skipped.slice(0, 5).map((s: any) => `   - ${s.mondayId}: ${s.reason}`).join('\n')}
${report.skipped.length > 5 ? `   ... et ${report.skipped.length - 5} autres` : ''}
` : ''}

${report.errors.length > 0 ? `❌ ERREURS (${report.errors.length} items)
${report.errors.slice(0, 5).map((e: any) => `   - ${e.mondayId}: ${e.error}`).join('\n')}
...
${report.missingFields.slice(0, 3).map((m: any) => `   - ${m.mondayId}: ${m.fields.join(', ')}`).join('\n')}
