...
export function createAdminRouter(storage: IStorage, eventBus: EventBus): Router {
    ...
    asyncHandler(async (req: Request, res: Response) => {
      return withErrorHandling(
        async () => {
          ...
          const stats = await (DateIntelligenceRulesSeeder as any).getRulesStatistics();
          ...
          await (DateIntelligenceRulesSeeder as any).seedDefaultRules();
          
          const updatedStats = await (DateIntelligenceRulesSeeder as any).getRulesStatistics();
          
          logger.info('[Admin] Seeding règles terminé', { metadata: {
            totalRules: updatedStats.totalRules,
            activeRules: updatedStats.activeRules,
            userId: req.user?.id
          }
        });
          
          sendSuccess(res, updatedStats);
        },
        {
          operation: 'getRulesStatistics',
          service: 'admin',
          metadata: {}
        }
      );
   ...
   * Réinitialiser toutes les règles de date intelligence
   ...
   * Valider toutes les règles de date intelligence
   * 
    ...
    asyncHandler(async (req: Request, res: Response) => {
      return withErrorHandling(
        async () => {
          ...
          const validation = await (DateIntelligenceRulesSeeder as any).validateRulesConsistency();
    ...
    asyncHandler(async (req: Request, res: Response) => {
      return withErrorHandling(
        ...
        const [rulesStats, rulesValidation] = await Promise.all([
          (DateIntelligenceRulesSeeder as any).getRulesStatistics(),
          (DateIntelligenceRulesSeeder as any).validateRulesConsistency()
        ]);
      ...
      {
        operation: 'getIntelligenceHealth',
        service: 'admin',
        metadata: {}
      }
    );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      return withErrorHandling(
        async () => {
          ...
          const testModule = await import('../../test/dateIntelligenceIntegration.test');
          const runIntegrationTest = (testModule as { runIntegrationTest?: () => Promise<unknown> }).runIntegrationTest;
          
          if (!runIntegrationTest) {
            throw createError.notFound('Test d\'intégration non disponible');
          }
          ...
          logger.info('[Admin] Test intégration terminé', { metadata: {
            success: (testResults as { success?: boolean }).success,
            healthScore: (testResults as { healthScore?: number }).healthScore || 0,
            eventsReceived: (testResults as { eventsReceived?: number }).eventsReceived || 0
          }
        });
    ...
    asyncHandler(async (req, res) => {
      return withErrorHandling(
        ...
        logger.info('Récupération règles avec filtres', { metadata: {
          ...
          if (phase) {
            filteredRules = filteredRules.filter(r => r.phase === phase);
          }
          if (projectType) {
            filteredRules = filteredRules.filter(r => r.projectType === projectType);
          }
          if (isActive !== undefined) {
            const isActiveValue = typeof isActive === 'string' 
              ? (isActive === 'true' || isActive === '1')
              : Boolean(isActive);
            filteredRules = filteredRules.filter(r => r.isActive === isActiveValue);
          }
          if (priority) {
            const minPriority = Number(priority);
            filteredRules = filteredRules.filter(r => r.priority && r.priority >= minPriority);
          }
      ...
      {
        operation: 'getIntelligenceRules',
        service: 'admin',
        metadata: {}
      }
    );
   ...
   * 
   * Création d'une nouvelle règle d'intelligence temporelle
   * 
   * @access authenticated
   */
  router.post("/api/intelligence-rules",
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      {
        operation: 'createIntelligenceRule',
        service: 'admin',
        metadata: {}
      }
    );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      return withErrorHandling(
        async () => {
          ...
          logger.info('[Admin] Configuration synchronisation mise à jour', { metadata: {
