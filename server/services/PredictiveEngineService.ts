...
export class PredictiveEngineService {
   ...
   */
  private predictDelaysAndOverruns(project: Project, delayHistory: unknown[]): { delay: number; overrun: number } {
    // Analyse basée sur l'historique de projets similaires
    const similarProjects = delayHistory.filter((h: unknown) => {
      const history = h as { project_type?: string };
      return history.project_type === project.menuiserieType;
    });
    let avgDelay = 0;
    let avgOverrun = 0;
    if (similarProjects.length > 0) {
      avgDelay = similarProjects.reduce((sum, p) => {
        const project = p as { delay_days?: number };
        return sum + (project.delay_days || 0);
   ...
   */
  private filterProjectsByParams(projects: unknown[], params: unknown): unknown[] {
    let filtered = projects;
    const paramsData = params as { project_types?: string[]; user_ids?: string[] };

    if (paramsData.project_types) {
      filtered = filtered.filter((p: unknown) => {
        const project = p as { menuiserieType?: string };
        return project.menuiserieType && paramsData.project_types!.includes(project.menuiserieType);
      });
    }

    if (paramsData.user_ids) {
      filtered = filtered.filter((p: unknown) => {
        const project = p as { responsibleUserId?: string };
        return project.responsibleUserId && paramsData.user_ids!.includes(project.responsibleUserId);
      });
    }

    // Seulement projets actifs
    filtered = filtered.filter((p: unknown) => {
      const project = p as { status?: string };
      return project.status && !['termine', 'archive', 'sav'].includes(project.status);
    });
   ...
   */
  private async getCurrentKPIs(period: unknown): Promise<unknown> {
    return withErrorHandling(
    async () => {
  ...
  private async generateRevenueRecommendations(currentKPIs: unknown, benchmarks: unknown): Promise<BusinessRecommendation[]> {
    const recommendations: BusinessRecommendation[] = [];
    const kpis = currentKPIs as { conversion_rate?: number };
    const bench = benchmarks as { industry_avg_conversion?: number };
    if (kpis.conversion_rate && bench.industry_avg_conversion && kpis.conversion_rate < bench.industry_avg_conversion) {
      const gap = bench.industry_avg_conversion - kpis.conversion_rate;
      recommendations.push({
        id: `revenue_conversion_${Date.now()}`,
        category: 'revenue',
        title: 'Améliorer le taux de conversion AO → Offres',
        description: `Taux actuel: ${kpis.conversion_rate}% vs benchmark: ${bench.industry_avg_conversion}%`,
   ...
   */
  private async generateCostOptimizationRecommendations(currentKPIs: unknown, benchmarks: unknown): Promise<BusinessRecommendation[]> {
    const recommendations: BusinessRecommendation[] = [];
    const kpis = currentKPIs as { avg_project_duration?: number };
    const bench = benchmarks as { avg_duration_benchmark?: number };
    if (kpis.avg_project_duration && bench.avg_duration_benchmark && kpis.avg_project_duration > bench.avg_duration_benchmark) {
      const excessDays = kpis.avg_project_duration - bench.avg_duration_benchmark;
      recommendations.push({
        id: `cost_duration_${Date.now()}`,
        category: 'costs',
        title: 'Réduire durée moyenne des projets',
        description: `Durée actuelle: ${kpis.avg_project_duration}j vs benchmark: ${bench.avg_duration_benchmark}j`,
  ...
  private async generateQualityRecommendations(currentKPIs: unknown): Promise<BusinessRecommendation[]> {
    const recommendations: BusinessRecommendation[] = [];
    const kpis = currentKPIs as { team_efficiency?: number };
    // Exemple de recommandation qualité basée sur les données
    if (kpis.team_efficiency && kpis.team_efficiency < 75) {
      recommendations.push({
        id: `quality_efficiency_${Date.now()}`,
        category: 'quality',
        title: 'Améliorer efficacité équipe',
        description: `Efficacité actuelle: ${kpis.team_efficiency}% (cible: 80%+)`,
   ...
   */
  private calculateConfidence(historical: unknown[], forecasts: unknown[]): number {
    if (historical.length < 3) return 40; // Confiance faible si peu de données
    // Calcul variance données historiques
    const revenues = historical.map((h: unknown) => {
      const data = h as { total_revenue?: number };
   ...
   */
  private identifyUnderlyingFactors(historical: unknown[]): string[] {
   ...
   */
  private calculateVolatility(historical: unknown[]): number {
    ...
    return mean > 0 ? Math.min(100, Math.round((Math.sqrt(variance) / mean) * 100)) : 50;
  }

  /**
   * Détecte direction de tendance
   */
  private detectTrendDirection(historical: unknown[]): 'up' | 'down' | 'stable' {
    if (historical.length < 3) return 'stable';
    
    const recent = historical.slice(-3);
    const revenues = recent.map((h: unknown) => {
      const data = h as { total_revenue?: number };
      return data.total_revenue || 0;
    });
