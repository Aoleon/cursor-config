...
/**
 * Script de correction automatique de TOUS les patterns metadata: { ... }); dans PredictiveEngineService.ts
 * 
 * Corrige: metadata: { ... }); -> metadata: { ... } });
 * Utilise une approche plus agressive pour détecter tous les patterns
 */
...
const TARGET_FILE = path.join(__dirname, '../server/services/PredictiveEngineService.ts');

function main() {
  console.log('🔧 Correction automatique de TOUS les patterns metadata dans PredictiveEngineService.ts...\n');
  ...
  // Pattern 1: Corriger metadata: { ... }); -> metadata: { ... } });
  ...
  if (content !== originalContent) {
    ...
    console.log(`\n✅ ${correctionsCount} corrections appliquées dans PredictiveEngineService.ts\n`);
  } else {
