...
export class MondayMigrationServiceEnhanced {
  ...
  async migrate(options: MigrationOptions): Promise<MigrationReport> {
    ...
    async () => {
      ...
      logger.info('Items fetched depuis Monday', { metadata: {
          service: 'MondayMigrationServiceEnhanced',
          operation: 'migrate',
...
service: 'MondayMigrationServiceEnhanced',
      ...
      report.completedAt = new Date();
      report.duration = report.completedAt.getTime() - startedAt.getTime();
      logger.error('Erreur migration Monday → Saxium', { metadata: {
          service: 'MondayMigrationServiceEnhanced',
          operation: 'migrate',
  ...
  async fetchAllItems(
    ...
    logger.info('Fetch items Monday.com avec pagination', { metadata: {
        service: 'MondayMigrationServiceEnhanced',
    ...
    async () => {
      ...
      logger.info('Fetch terminé', { metadata: {
          service: 'MondayMigrationServiceEnhanced',
          operation: 'fetchAllItems',
 ...
 : unknown[]s: unknown[],
    ...
    logger.info('Démarrage bulk insert (parallel batches)', { metadata: {
        service: 'MondayMigrationServiceEnhanced',
    ...
    for (let i = 0; i < items.length; i += PARALLEL_BATCH_SIZE) {
      ...
      logger.info(`Batch ${batchNumber}/${totalBatches}: ${processedSoFar}/${items.length} items (${percentage}%)`, { metadata: {
          service: 'MondayMigrationServiceEnhanced',
          operation: 'bulkInsert',
          batchNumber,
          totalBatches,
  ...
  private extractColumnValues(item: MondayItem): Map<string, unknown> {
    const values = new Map<st, unknown>unknown>();
...
export function getMondayMigrationServiceEnhanced(storage: IMigrationStorage): MondayMigrationServiceEnhanced {
  if (!enhancedService) {
    enhancedService = mondaymigrationServiceEnhanced(storage);
  }
