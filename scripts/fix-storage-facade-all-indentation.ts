#!/usr/bin/env tsx
/**
 * Script pour corriger toutes les indentations excessives restantes dans StorageFacade.ts
 */
...
function fixAllIndentation(content: string): { fixed: number; content: string } {
  let fixed = 0;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Pattern 1: Fermeture de metadata avec trop d'indentation (12+ espaces) suivie de }); avec trop d'indentation
    //                }
    //                                      });
    // Devrait être:
    //        }
    //      });
    if (line.match(/^\s{12,}\}$/) && nextLine.match(/^\s{12,}\}\);$/)) {
    ...
    // Pattern 2: Fermeture avec trop d'indentation (12+ espaces) seule
    //              }
    // Devrait être:
    //        }
    if (line.match(/^\s{12,}\}$/)) {
    ...
    // Pattern 3: }); avec trop d'indentation (12+ espaces) seule
    //                                      });
    // Devrait être:
    //      });
    if (line.match(/^\s{12,}\}\);$/)) {
...
async function main() {
  console.log('🔧 Correction de toutes les indentations excessives dans StorageFacade.ts...\n');

  try {
    const originalContent = readFileSync(filePath, 'utf-8');
    const result = fixAllIndentation(originalContent);
