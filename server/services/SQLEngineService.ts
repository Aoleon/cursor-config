...
export class SQLEngineService {
  ...
  private fixCommonSQLTypos(sql: string): string {
    ...
    if (correctedSQL !== sql) {
      logger.info('Corrections SQL appliquées', { metadata: {
  ...
  private cleanGeneratedSQL(sql: string): string {
    ...
    if (cleanedSQL !== originalSQL) {
      logger.warn('Nettoyage du SQL généré - colonnes corrigées', { metadata: {
          service: 'SQLEngineService',
  ...
  async executeNaturalLanguageQuery(request: SQLQueryRequest): Promise<SQLQueryResult> {
    ...
    async () => {
      ...
      logger.info('Analyse d\'intention reçue', { metadata: {
          service: 'SQLEngineService',
          operation: 'executeNaturalLanguageQuery',
          queryType,
      ...
      logger.info('SQL généré par l\'IA', { metadata: {
        service: 'SQLEngineService',
        operation: 'executeNaturalLanguageQuery',
...
LIMIT 25;`);
  ...
  async validateGeneratedSQL(sql: string, queryType: string): Promise<{
    ...
    async () => {
      ...
      logger.info('Validation SQL détaillée', { metadata: {
          service: 'SQLEngineService',
...
`;
  ...
  private async validateSQLSecurity(sql: string, userId: string, userRole: string): Promise<{
    ...
    logger.info('Validation SQL', { metadata: {
        service: 'SQLEngineService',
        operation: 'validateSQL',
        userId,
        userRole
    ...
    async () => {
      ...
      logger.info('SQL prétraité pour parsing', { metadata: {
          service: 'SQLEngineService',
          operation: 'validateSQL',
    ...
    logger.info('Résultat validation SQL', { metadata: {
        service: 'SQLEngineService',
        operation: 'validateSQL',
   ...
   */
  private extractColumnsFromAST: unknown): Array<{table?: string, column: string}> {
  ...
  private validateBusinessConstraints: unknown, unknown, userRole: string, violations: string[]): void {
    ...
    if (userRole !== 'admin') {
      ...
      if (!hasUserFilter) {
        ...
        logger.info('Filtre user_id manquant, sera ajouté par RBAC', { metadata: {
        service: 'SQLEngineService',
  ...
  private async applyRBACFilters(
    ...
    async () => {
      ...
      logger.info('Filtres RBAC appliqués avec succès', { metadata: {
          service: 'SQLEngineService',
          operation: 'applyRBACFilters',
          filtersApplied,
          userRole 
