...
export class MondaySchemaAnalyzer {
  ...
  async analyzeBoards(boardIds?: string[]): Promise<BoardAnalysisResult> {
    logger.info('Analyse structure boards Monday.com', {
      metadata: {
    ...
    async () => {
      ...
      logger.info('Analyse boards terminée', {
        metadata: {
          service: 'MondaySchemaAnalyzer',
          operation: 'analyzeBoards',
  ...
  async analyzeBoardStructure(boardId: string, boardName?: string): Promise<BoardStructure> {
    ...
    if (cached) {
      logger.debug('Structure board récupérée depuis cache', {
        metadata: {
          service: 'MondaySchemaAnalyzer',
    ...
    logger.info('Analyse structure board', {
      metadata: {
        service: 'MondaySchemaAnalyzer',
        operation: 'analyzeBoardStructure',
        boardId,
    ...
    async () => {
      ...
      logger.info('Structure board analysée et mise en cache', {
        metadata: {
          service: 'MondaySchemaAnalyzer',
          operation: 'analyzeBoardStructure',
          boardId,
  ...
  async suggestMappings(
    ...
    logger.info('Génération suggestions mapping', {
      metadata: {
        service: 'MondaySchemaAnalyzer',
    ...
    logger.info('Suggestions mapping générées', {
      metadata: {
        service: 'MondaySchemaAnalyzer',
        operation: 'suggestMappings',
  ...
  private parseSettings(settingsStr: string): Record<string, unknown> {
    try {
      ...
      logger.warn('Impossible de parser settings JSON', {
        metadata: {
          service: 'MondaySchemaAnalyzer',
  ...
  async invalidateBoardCache(boardId: string): Promise<void> {
    ...
    logger.info('Cache board invalidé', {
      metadata: {
        service: 'MondaySchemaAnalyzer',
