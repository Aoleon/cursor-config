...
/**
 * Script de correction finale des placeholders operation: 'async',
 * en remplaçant par le nom réel de la méthode
 * Version finale ciblant les 17 occurrences restantes
 */
      ...
      if (lines[i].includes("operation: 'async',")) {
        ...
        for (let j = i - 1; j >= Math.max(0, i - 150); j--) {
          ...
          if (methodMatch) {
            ...
            const keywords = ['if', 'for', 'while', 'switch', 'catch', 'then', 'catch', 'return', 'await', 'const', 'let', 'var'];
        ...
        if (foundMethod && methodName) {
          ...
          // Si on ne trouve pas, utiliser un nom générique basé sur le contexte
          // Chercher le nom de la fonction qui contient ce withErrorHandling
          for (let j = i - 1; j >= Math.max(0, i - 200); j--) {
            const functionMatch = lines[j].match(/(?:async\s+)?(?:private\s+|public\s+|protected\s+)?(\w+)\s*\([^)]*\)\s*[:=]/);
            if (functionMatch) {
              const funcName = functionMatch[1];
              if (!['if', 'for', 'while', 'switch', 'catch', 'then', 'catch', 'return', 'await', 'const', 'let', 'var'].includes(funcName)) {
                lines[i] = lines[i].replace("operation: 'async',", `operation: '${funcName}',`);
                fixCount++;
                break;
              }
            }
...
async function main() {
  ...
  console.log('🔧 Correction des placeholders operation: async (version finale)...\n');
