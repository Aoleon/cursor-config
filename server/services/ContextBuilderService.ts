...
case 'client':;
  ...
  private classifyProjectSize(am: unknown): 'small' | 'medium' | 'large' {
...
case 'client':;
    ...
    // 6. Estimation tokens
    contextData.tokenEstimate = this.estimateTokens(contextData);
    logger.info('Contexte sélectif construit', { metadata: {
        service: 'ContextBuilderService',
        operation: 'buildSelectiveContext',
        tokenEstimate: contextData.tokenEstimate   
              }
  ...
  private async buildOfferContextLimited(
    ...
    logger.info('Construction offre limitée', { metadata: {
        service: 'ContextBuilderService',
  ...
  private async buildProjectContextLimited(
    ...
    logger.info('Construction projet limitée', { metadata: {
        service: 'ContextBuilderService',
  ...
  private async buildSupplierContextLimited(
    ...
    logger.info('Construction fournisseur limitée', { metadata: {
        service: 'ContextBuilderService',
  ...
  private async buildTeamContextLimited(
    ...
    logger.info('Construction équipe limitée', { metadata: {
        service: 'ContextBuilderService',
  ...
  private async buildClientContextLimited(
    ...
    logger.info('Construction client limitée', { metadata: {
        service: 'ContextBuilderService',
  ...
  private async enrichContextSelectively(
    ...
    logger.info('Enrichissement sélectif types', { metadata: {
        service: 'ContextBuilderService',
...
export function resetContextBuilderService(): void {
  ...
  logger.info('Service réinitialisé - prêt pour nouvelle configuration', { metadata: {
      service: 'ContextBuilderService',
